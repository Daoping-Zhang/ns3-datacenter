...
class SwitchMmu : public Object
{
  public:
    ...
    bool CheckIngressAdmission(uint32_t port,
                               uint32_t qIndex,
                               uint32_t psize,
                               uint32_t type,
                               uint32_t unsched);
    bool CheckEgressAdmission(uint32_t port,
                              ...
                              uint32_t type,
                              uint32_t unsched);
    void UpdateIngressAdmission(uint32_t port,
                                uint32_t qIndex,
                                uint32_t psize,
                                uint32_t type,
                                uint32_t unsched);
    ...
    void SetBufferModel(std::string model)
    {
        bufferModel = model;
    }
    ...
    uint64_t Threshold(uint32_t port,
                       uint32_t qIndex,
                       std::string inout,
                       uint32_t type,
                       uint32_t alphaPrio);
    ...
    uint64_t ActiveBufferManagement(uint32_t port,
                                    uint32_t qIndex,
                                    std::string inout,
                                    uint32_t type,
                                    uint32_t unsched);

    uint64_t FlowAwareBuffer(uint32_t port,
                             uint32_t qIndex,
                             std::string inout,
                             uint32_t type,
                             uint32_t unsched);

    void SetABMalphaHigh(double alpha)
    {
        alphaHigh = alpha;
    };

    void SetABMdequeueUpdateNS(double time)
    {
        updateIntervalNS = time;
    }

    void SetPortCount(uint32_t pc)
    {
        portCount = pc;
    }
