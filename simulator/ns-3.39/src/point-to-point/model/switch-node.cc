...
SwitchNode::SendToDev(Ptr<Packet> p, CustomHeader& ch)
{
    ...
    if (idx >= 0)
    {
        ...
        // IMPORTANT: MyPriorityTag should only be attached by lossy traffic. This tag indicates the
        // qIndex but also indicates that it is "lossy". Never attach MyPriorityTag on lossless
        // traffic.
        bool found = p->PeekPacketTag(priotag);

        // UnSchedTag is used by ABM. End-hosts explicitly tag packets of the first BDP so that ABM
        // then prioritizes these packets in the buffer allocation.
        uint32_t unsched = 0;
        UnSchedTag tag;
        ...
        {
            qIndex = (ch.l3Prot == 0x06 ? 1 : ch.udp.pg); // For TCP/IP if the stack did not attach
                                                          // MyPriorityTag, put to queue 1.
        }
        ...
        { // not highest priority
            // IMPORTANT: MyPriorityTag should only be attached by lossy traffic. This tag indicates
            // the qIndex but also indicates that it is "lossy". Never attach MyPriorityTag on
            // lossless traffic.
            if (m_mmu->CheckIngressAdmission(inDev, qIndex, p->GetSize(), found, unsched) &&
                m_mmu->CheckEgressAdmission(idx, qIndex, p->GetSize(), found, unsched))
            { // Admission control
        ...
        DynamicCast<QbbNetDevice>(m_devices[idx])->totalBytesRcvd +=
    ...
    if (1)
    {
        ...
        case 0x11: { // udp packet
            ...
            case CC_MODE::HPCC_PINT: {
                ...
                int b = 20;
                int m = 16;
                int l = 20; // see log2apprx's paremeters
